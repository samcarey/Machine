...
forearm_l = paddle_upper_l+wrist_joint_outer+slide_gap;
...
module forearm(){
	...
	translate([0,forearm_l/2,0]){
		...
		translate([bolt_h/4+0.1,0,0]){
			difference(){
				bolt();
				translate([bolt_h*0.75,0,0])
				cube([bolt_h,bolt_r*2+0.1,bolt_groove],center = true);
			}
		}
	}
	translate([forearm_t/2,-forearm_l/2+clip_l+thumb_hook_l])
		clip();
}
